...
import { BarChart3, Zap, Users, EyeOff, Eye, MessageSquare, Smartphone, Check, CheckCircle2, ArrowRight, X } from 'lucide-react';
  ...
  return (
    <>
      <style>{`
        ...
        .strength-bar.good   { background: var(--color-accent); }
      ...
      <div className={`h-screen w-full flex overflow-hidden transition-opacity duration-300 ${fadeout ? 'opacity-0' : 'opacity-100'}`}>
        ...
        <div className="w-full md:w-[45%] h-full bg-[#F5F5F3] overflow-y-auto overflow-x-hidden relative flex flex-col items-center pt-[var(--nav-height)]">
          {/* Form Container */}
          <motion.div 
            ...
            {mode !== '#forgot' ? (
              <>
                ...
                {mode === '#login' && (
                  <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
                    ...
                    <div className="space-y-4 mb-8">
                      <button className="w-full h-[52px] bg-[#07C160] hover:bg-[#06A852] text-white rounded-[12px] font-normal flex items-center justify-center gap-2 transition-colors text-[16px]">
                        <MessageSquare className="w-5 h-5" /> 微信扫码登录
                      </button>
                      <button className="w-full h-[52px] bg-[#E5E5E3] hover:bg-[#DCDCDA] text-[#080808] rounded-[12px] font-normal flex items-center justify-center gap-2 transition-colors text-[16px]">
                    ...
                    <form onSubmit={handleLogin} className="space-y-4">
                      <div>
                        <input 
                          ...
                          className="w-full h-[52px] px-4 bg-white rounded-[12px] text-[#080808] placeholder:text-[#9B9B96] focus:outline-none focus:ring-4 focus:ring-black/5 transition-all text-[15px]"
                        />
                      </div>
                      <div className="relative">
                        <input 
                          ...
                          className="w-full h-[52px] pl-4 pr-12 bg-white rounded-[12px] text-[#080808] placeholder:text-[#9B9B96] focus:outline-none focus:ring-4 focus:ring-black/5 transition-all text-[15px]"
                        />
                        <button 
                          type="button" 
                      ...
                      <button 
                        ...
                        className={`w-full h-[52px] rounded-[12px] font-normal text-white transition-colors duration-300 mt-6 flex items-center justify-center gap-2 text-[16px] ${
                      ...
                      <div className="text-center pt-4">
                        <button type="button" onClick={() => setMode('#register')} className="text-[14px] text-[#444444]">
                          没有账号？ <span className="text-[#080808] font-normal">立即注册</span>
                        </button>
                      </div>
                    </form>
                  </div>
                  ...
                  <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
                    <form onSubmit={handleRegister} className="space-y-4">
                      ...
                      <button 
                        type="submit" 
                        disabled={loading || success}
                        className={`w-full h-[52px] rounded-[12px] font-normal text-white transition-colors duration-300 mt-2 flex items-center justify-center gap-2 text-[16px] ${
               ...
               <div className="animate-in fade-in slide-in-from-bottom-2 duration-300 w-full">
                 <button onClick={() => setMode('#login')} className="text-[#9B9B96] hover:text-[#080808] mb-8 text-[14px] flex items-center gap-1 font-normal transition-colors">
                    &larr; 返回登录
                 </button>
                 
                 <h2 className="text-[28px] font-normal text-[#080808] mb-2">重置密码</h2>
                 ...
                 {forgotStep === 1 && (
                   <form autoFocus onSubmit={handleForgotNext} className="space-y-4 animate-in fade-in zoom-in-95 duration-300">
                     <h3 className="text-[18px] font-normal text-[#080808] mb-4">第一步：验证身份</h3>
                     <input 
                        type="text" 
                      ...
                      <button 
                        type="submit" 
                        disabled={loading}
                        className="w-full h-[52px] bg-[#080808] hover:bg-[#1A1A1A] text-white rounded-[12px] font-normal transition-colors mt-4 disabled:opacity-70 text-[16px]"
                      >
                        {loading ? '验证中...' : '下一步'}
                      </button>
                   </form>
                 ...
                 {forgotStep === 2 && (
                   <form onSubmit={handleForgotNext} className="space-y-4 animate-in fade-in zoom-in-95 duration-300">
                     <h3 className="text-[18px] font-normal text-[#080808] mb-4">第二步：设置新密码</h3>
                     <div className="relative">
                        <input 
                          type={showPassword ? "text" : "password"} 
                          placeholder="新密码" 
                      ...
                      <button 
                        type="submit" 
                        disabled={loading}
                        className="w-full h-[52px] bg-[#080808] hover:bg-[#1A1A1A] text-white rounded-[12px] font-normal transition-colors mt-6 disabled:opacity-70 text-[16px]"
                      >
                        {loading ? '提交中...' : '确认修改'}
                      </button>
                   </form>
                 ...
                 {forgotStep === 3 && (
                   <div className="text-center animate-in fade-in zoom-in-95 duration-300 py-12 flex flex-col items-center">
                     <div className="w-16 h-16 bg-[#22C97A] rounded-full flex items-center justify-center text-white mb-6 border-none">
                       <Check className="w-8 h-8" />
                     </div>
                     <h3 className="text-[22px] font-normal text-[#080808] mb-2">密码已重置</h3>
                     <p className="text-[#9B9B96] mb-8">正在为您跳转回登录界面 ({forgotRedirectCount}s)</p>
                     
                     <button 
                       onClick={() => setMode('#login')}
                       className="w-full h-[52px] bg-[#080808] hover:bg-[#1A1A1A] text-white rounded-[12px] font-normal transition-colors text-[16px]"
                     >
                       立即登录
                     </button>
                   </div>
        ...
        <div className="hidden md:flex w-[55%] h-full bg-[#080808] relative items-center justify-center overflow-hidden">
          ...
          <div className="relative z-10 w-full max-w-[520px] px-8">
             <h2 className="text-[54px] font-normal text-white leading-[1.1] mb-12 tracking-[-1.5px] font-sans">
               加入100万+<br />健康感知者
             </h2>
             ...
             <div className="space-y-8 mb-16">
               <div className="flex gap-5 items-start">
                 ...
                 <div>
                    <h3 className="text-[16px] font-normal text-white mb-2 tracking-wide">完整健康数据历史</h3>
               ...
               <div className="flex gap-5 items-start">
                 ...
                 <div>
                    <h3 className="text-[16px] font-normal text-white mb-2 tracking-wide">AI个性化解读</h3>
               ...
               <div className="flex gap-5 items-start">
                 ...
                 <div>
                    <h3 className="text-[16px] font-normal text-white mb-2 tracking-wide">家庭健康管理</h3>
             ...
             <div className="flex items-center gap-4 pt-8 border-t border-[rgba(255,255,255,0.1)]">
                <div className="flex -space-x-3">
                  ...
                  <div className="w-8 h-8 rounded-full bg-[#1A1A1A] flex items-center justify-center text-[10px] text-white font-normal">+</div>
                </div>
                <span className="text-[14px] text-[rgba(255,255,255,0.6)] font-normal tracking-wide">
                  已有 1,024,847 人在使用
                </span>
             </div>
          </div>
        ...
        {registerModalOpen && (
          ...
          >
            <motion.div 
              ...
              className="relative flex w-full max-w-[480px] flex-col items-center rounded-[12px] bg-[#1A1A1A] p-8 text-center md:p-12"
            >
              <button
                type="button"
              ...
              {/* Coupon Gift */}
              <div className="w-full bg-[rgba(200,255,0,0.05)] rounded-[12px] p-4 mb-10 flex items-center justify-between group hover:bg-[rgba(200,255,0,0.1)] transition-colors cursor-pointer">
                <div className="flex flex-col items-start">
                  <span className="text-accent font-normal text-[14px] mb-1">新注册用户专属福利</span>
              ...
              <div className="w-full space-y-4">
                <button 
                  ...
                  className="w-full h-[52px] bg-accent text-ink font-normal text-[16px] rounded-[12px] hover:bg-white transition-colors"
                >
                  开始探索
                </button>
                <button 
                  ...
                  className="text-[#9B9B96] hover:text-white transition-colors text-sm font-normal"
                >
                  稍后再说
                </button>
              </div>
