...
import { ChevronRight, ArrowRight, Download, Mail, Share2, Plus } from 'lucide-react';
    ...
    >
      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-center gap-3">
          <span className="text-[#86868B] text-[10px] font-normal tracking-widest uppercase">{article.category}</span>
          <span className="text-[#A1A1A6] text-[12px]">{article.date}</span>
        </div>
        <h3 className={`text-[#0D0D0D] font-normal leading-[1.2] transition-colors ${type === 'hero' ? 'text-[24px] md:text-5xl tracking-[-0.03em]' : 'text-[18px]'}`}>
          {article.title}
        </h3>
        {type === 'hero' && article.summary && (
          <p className="text-black/60 text-[14px] md:text-lg leading-[1.6] line-clamp-2 max-w-[800px]">
            {article.summary}
          </p>
        )}
        <div className="mt-4 flex items-center justify-between">
          <span className="text-black/60 text-[12px]">作者: {article.author} · {article.readingTime} 阅读</span>
          <span className="text-[#0D0D0D] font-normal text-[12px] transform transition-transform group-hover:translate-x-1">→</span>
        </div>
      </div>
    </motion.div>
  );
...
const HeaderSection = () => {
  ...
  return (
    <section className="relative h-[85vh] w-full bg-[#080808] overflow-hidden flex items-end">
      ...
      <div className="w-full px-4 md:px-[170px] pb-10 md:pb-24 relative z-10">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="max-w-[1000px] flex flex-col gap-8">
          <h1 className="text-white text-[32px] md:text-[80px] font-normal leading-[1.1] tracking-[-0.04em]">
            {MOCK_ARTICLES[0].title}
          </h1>
          <p className="text-white/60 text-[14px] md:text-2xl font-normal leading-[1.6] max-w-[800px]">
            {MOCK_ARTICLES[0].summary}
          </p>
          <button 
            onClick={() => navigate(withPath(`/news/${MOCK_ARTICLES[0].id}`))}
            className="group w-full md:w-fit bg-accent text-ink px-6 py-3 rounded-full text-[14px] font-normal flex items-center justify-center gap-2 transition active:scale-95"
          >
...
const ContentFilterBar = ({ activeTab, onTabChange }: { activeTab: string, onTabChange: (id: string) => void }) => {
  return (
    <div className="sticky top-[var(--nav-height)] z-[90] bg-white/80 backdrop-blur-xl h-[64px] flex items-center">
      <div className="w-full px-4 md:px-[170px] mx-auto flex items-center">
        <div className="flex items-center gap-10 overflow-x-auto hide-scrollbar">
          {CATEGORY_TABS.map((tab) => (
            <button 
              key={tab.id}
              onClick={() => onTabChange(tab.id)}
              className={`relative h-[64px] flex flex-col justify-center transition-colors ${activeTab === tab.id ? 'text-[#0D0D0D] font-normal' : 'text-black/50'}`}
            >
              <span className="text-[12px] md:text-sm tracking-tight">{tab.name}</span>
              {activeTab === tab.id && <motion.div layoutId="tabUnder" className="absolute bottom-0 left-0 right-0 h-[3px] bg-[#0D0D0D]" />}
            </button>
          ))}
        </div>
      </div>
      <div className="absolute inset-x-0 bottom-0 h-px bg-black/5" aria-hidden />
    </div>
  );
};
...
const EditorialPicks = () => {
  ...
  const { withPath } = useLocalePath();
  return (
    <section className="bg-white pt-8 pb-6 md:py-32">
      <div className="w-full px-4 md:px-[170px] mx-auto flex flex-col gap-6 md:gap-16">
        <div className="pb-4 flex items-end justify-between">
          <div className="flex flex-col gap-2">
            <h2 className="text-[#0D0D0D] text-[24px] md:text-4xl font-normal tracking-[-0.03em]">精选推荐</h2>
          </div>
        </div>
            ...
            {[MOCK_ARTICLES[2], MOCK_ARTICLES[3]].map((a, i) => (
              <div key={a.id} onClick={() => navigate(withPath(`/news/${a.id}`))} className="grid grid-cols-5 gap-8 group cursor-pointer items-center">
                ...
                <div className="col-span-3 flex flex-col justify-center gap-3">
                  <span className="text-[#86868B] text-[10px] font-normal uppercase tracking-widest">{a.category}</span>
                  <h4 className="text-[#0D0D0D] text-[18px] md:text-xl font-normal leading-[1.2] group-hover:text-black/60 transition-colors">{a.title}</h4>
                  <span className="text-black transform transition-transform group-hover:translate-x-1 font-normal text-sm">→</span>
                </div>
              </div>
            ))}
          </div>
...
const ResearchReports = () => {
  ...
  return (
    <section className="bg-white pt-8 pb-6 md:py-32 px-4 md:px-[170px]">
      <div className="flex flex-col gap-16">
        <div className="max-w-[800px] flex flex-col gap-4">
          <h2 className="text-[#0D0D0D] text-[24px] md:text-5xl font-normal tracking-[-0.03em] leading-[1.1]">用数据说话,<br/>这是我们的深度发现</h2>
          <p className="text-black/60 text-[14px] md:text-xl leading-[1.6]">通过大规模匿名调查与数据分析,理解时代的健康脉搏。</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-stretch">
          {reports.map((r, i) => (
            <a 
              ...
              href={r.link}
              download
              className={`${r.isPrimary ? 'md:col-span-6 bg-[#F5F5F3] text-[#0D0D0D]' : 'md:col-span-3 bg-[#F5F5F3] text-[#0D0D0D]'} rounded-[12px] p-4 md:p-10 flex flex-col justify-between transition-colors group cursor-pointer`}
            >
              <div className="flex flex-col gap-6">
                <span className={`${r.isPrimary ? 'text-black/40' : 'text-black/50'} text-[12px] font-normal tracking-[0.12em] uppercase`}>{r.date}</span>
                <h3 className={`text-[18px] md:text-2xl font-normal leading-[1.2] ${r.isPrimary ? 'md:text-4xl md:pr-8' : ''}`}>{r.title}</h3>
              </div>
              <div className="mt-6 md:mt-12 flex items-center justify-between pt-4 md:pt-6 text-[12px] font-normal uppercase tracking-[0.12em]">
                <span className="text-black/60">PDF 下载</span>
...
const ArticleDetail = ({ article, onBack }: { article: Article, onBack: () => void }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white min-h-screen pt-24 pb-10 md:pb-40 px-4 md:px-[170px] font-sans">
      <div className="max-w-[900px] mx-auto">
        <button onClick={onBack} className="text-black/60 hover:text-[#0D0D0D] flex items-center gap-2 mb-6 font-normal text-[14px]"><ChevronRight className="rotate-180 w-4 h-4" /> 返回资讯中心</button>
        <div className="flex flex-col gap-6 md:gap-8 mb-12 md:mb-16">
          <span className="text-black/50 text-[12px] font-normal tracking-[0.12em] uppercase">{article.tag}</span>
          <h1 className="text-[#0D0D0D] text-[32px] md:text-6xl font-normal leading-[1.1] tracking-[-0.04em]">{article.title}</h1>
          <div className="h-px bg-black/10" aria-hidden />
          <div className="flex items-center gap-4 py-2 md:py-4">
            <div className="w-12 h-12 rounded-full bg-[#0D0D0D] flex items-center justify-center font-normal text-white">A</div>
            <div className="flex flex-col">
              <span className="font-normal text-black">{article.author}</span>
              <span className="text-black/50 text-[12px]">{article.date} · {article.readingTime} 阅读量</span>
            </div>
            <div className="ml-auto flex gap-3">
              <button className="p-3 rounded-full bg-black/5 hover:bg-black/10 transition-colors"><Share2 className="w-4 h-4" /></button>
              <button className="p-3 rounded-full bg-black/5 hover:bg-black/10 transition-colors"><Plus className="w-4 h-4" /></button>
            </div>
          </div>
        </div>
        ...
        <div className="mt-10 md:mt-40 pt-8 md:pt-24 flex flex-col gap-6 md:gap-12">
          <div className="h-px bg-black/10" aria-hidden />
          <h3 className="text-[#0D0D0D] text-[12px] font-normal tracking-[0.12em] uppercase">继续阅读 KEEP READING</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {MOCK_ARTICLES.slice(1, 4).map(a => (
              <div key={a.id} onClick={() => {}} className="cursor-pointer group">
                ...
                <h4 className="text-[18px] font-normal line-clamp-2 text-[#0D0D0D] group-hover:text-black/60 transition-colors">{a.title}</h4>
              </div>
            ))}
          </div>
        </div>
...
const BrandChronicle = () => {
    ...
    return (
        <section ref={scrollRef} className="bg-[#0D0D0D] pt-8 pb-6 md:py-40 px-4 md:px-[170px] relative overflow-hidden">
            <div className="flex flex-col items-center gap-4 md:gap-8 mb-8 md:mb-32 text-center max-w-[800px] mx-auto">
                <h2 className="text-white text-[24px] md:text-6xl font-normal tracking-[-0.03em] leading-[1.1]">从一个想法 到一个时代</h2>
            </div>
                ...
                <div className="flex flex-col gap-12 md:gap-24 relative">
                    {milestones.map((m, i) => (
                        <div key={i} className={`flex items-center w-full ${m.side === 'left' ? 'flex-row' : 'flex-row-reverse'}`}>
                            <div className={`w-1/2 ${m.side === 'left' ? 'pr-8 md:pr-20 text-right' : 'pl-8 md:pl-20 text-left'}`}>
                                <motion.div initial={{ opacity: 0, x: m.side === 'left' ? -20 : 20 }} whileInView={{ opacity: 1, x: 0 }}>
                                    <span className="text-white text-[24px] font-normal mb-2 md:mb-4 block tracking-[-0.03em]">{m.year}</span>
                                    <h3 className="text-white text-[18px] md:text-2xl font-normal mb-2 md:mb-4">{m.title}</h3>
                                    <p className="text-white/55 text-[12px] md:text-[14px] leading-[1.6]">{m.desc}</p>
                                </motion.div>
                            </div>
                            <div className="absolute left-1/2 -translate-x-1/2 flex items-center justify-center">
                                <motion.div initial={{ scale: 0 }} whileInView={{ scale: 1 }} className="w-3 h-3 rounded-full bg-white z-10" />
                            </div>
                            <div className="w-1/2" />
                        </div>
                    ))}
...
export default function NewsPage() {
  ...
  return (
    <div className="bg-white min-h-screen font-sans">
      ...
      <section className="bg-white pt-8 pb-6 md:py-24 px-4 md:px-[170px]">
