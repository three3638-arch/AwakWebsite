...
export default function LanguageMenu({ tone, onNavigate }: Props) {
  ...
  const panelTone =
    tone === 'onLight'
      ? 'border-none bg-white text-[#1D1D1F]'
      : 'border-none bg-[#111111] text-white';
    ...
    <div className="relative" ref={rootRef}>
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            <div className={`flex justify-end border-b px-1 py-1 ${tone === 'onLight' ? 'border-black/10' : 'border-white/10'}`}>
              <button
                type="button"
                onClick={() => setOpen(false)}
                className={
                  tone === 'onLight'
                    ? 'rounded-full p-1.5 text-black/50 transition-colors hover:bg-black/[0.06] hover:text-black'
                    : 'rounded-full p-1.5 text-white/50 transition-colors hover:bg-white/10 hover:text-white'
                }
                aria-label="关闭"
              >
